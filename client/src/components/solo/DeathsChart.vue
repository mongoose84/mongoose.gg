<template>
  <div class="deaths-chart" data-testid="deaths-chart">
    <!-- Chart -->
    ...
    <div v-else class="empty-state" data-testid="empty-state">
      <p class="empty-text">No deaths data available</p>
      <p class="empty-subtext">Play some games to see your death trends</p>
    </div>
  </div>
</template>
...
// Get line color based on trend: green when improving (deaths decreasing), red when worsening
...
// Build annotation config for overall average reference line
...
const maxYValue = computed(() => {
  if (!hasData.value) return 15
  const maxDeaths = Math.max(...props.data.map(p => p.deaths))
  return Math.ceil(maxDeaths * 1.2)
})
...
const chartOptions = computed(() => ({
  ...
  scales: {
    ...
    y: {
      ...
      ticks: { 
        color: '#888888', 
        stepSize: Math.max(1, Math.ceil(maxYValue.value / 10)),
        font: { size: 11 } 
      }
    }
  }
