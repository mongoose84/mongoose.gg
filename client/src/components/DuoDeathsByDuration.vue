<template>
  <div class="deaths-duration-container">
    <div v-if="loading" class="loading">Loading deaths by duration…</div>
    <div v-else-if="error" class="error">{{ error }}</div>
    <div v-else-if="!hasData" class="empty">No death duration data available.</div>

    <ChartCard v-else title="📊 Deaths by Game Length">
      <div class="chart-content">
        ...
        <div class="legend">
          <span class="legend-item"><span class="dot low"></span> Low deaths</span>
          <span class="legend-item"><span class="dot high"></span> High deaths</span>
        </div>
      </div>
    </ChartCard>
  </div>
...
import getDuoDeathsByDuration from '@/assets/getDuoDeathsByDuration.js';
...
const maxDeaths = computed(() => {
  if (!data.value?.buckets) return 20;
  const max = Math.max(...data.value.buckets.map(b => b.avgDeaths));
  return Math.ceil(max / 5) * 5 || 20;
});
...
const bars = computed(() => {
  ...
  const plotHeight = chartHeight - padding.top - padding.bottom;
  const avgDeaths = buckets.reduce((sum, b) => sum + b.avgDeaths, 0) / buckets.length;

  return buckets.map((bucket, i) => {
    const x = padding.left + (plotWidth / buckets.length) * i + 7.5;
    const height = (bucket.avgDeaths / maxDeaths.value) * plotHeight;
    const isHigh = bucket.avgDeaths > avgDeaths;
    return {
      x, y: getY(bucket.avgDeaths), width: barWidth, height,
      value: bucket.avgDeaths.toFixed(1),
...
async function load() {
  ...
  error.value = null;
  try {
    data.value = await getDuoDeathsByDuration(props.userId);
  } catch (e) {
    error.value = e?.message || 'Failed to load deaths by duration data.';
  } finally {
    loading.value = false;
  }
}
...
<style scoped>
.deaths-duration-container { width: 100%; height: 100%; display: flex; flex-direction: column; }
.deaths-duration-container :deep(.chart-card) { flex: 1; display: flex; flex-direction: column; }
.loading, .error, .empty { padding: 2rem; text-align: center; color: var(--color-text-muted); }
.error { color: var(--color-danger); }
.chart-content { padding: 1rem 0; flex: 1; display: flex; flex-direction: column; }
.bar-chart { width: 100%; flex: 1; min-height: 120px; }
...
.legend { display: flex; justify-content: center; gap: 1.5rem; margin-top: auto; padding-top: 0.5rem; }
