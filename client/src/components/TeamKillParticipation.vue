    ...
    <ChartCard v-else title="🗡️ Kill Participation">
      <div class="share-content">
        ...
        <div class="total-header">
          <span class="total-label">Total Team Kills:</span>
          <span class="total-value">{{ data.totalTeamKills }}</span>
        </div>
        ...
        <p v-if="lowestPlayer" class="insight">
          ...
          <span v-else>
            ✓ Good kill participation across the team
          </span>
        </p>
      </div>
    </ChartCard>
...
import getTeamKillParticipation from '@/assets/getTeamKillParticipation.js';
...
async function load() {
  ...
  error.value = null;
  try {
    data.value = await getTeamKillParticipation(props.userId);
  } catch (e) {
