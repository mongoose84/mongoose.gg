...
import getTeamKillsTrend from '@/assets/getTeamKillsTrend.js';
...
const maxKills = computed(() => {
  ...
  const max = Math.max(...data.value.dataPoints.map(d => d.teamKills));
...
async function load() {
  ...
  loading.value = true;
  error.value = null;
  try { data.value = await getTeamKillsTrend(props.userId); }
