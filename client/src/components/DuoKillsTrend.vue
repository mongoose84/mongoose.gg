<template>
  <div class="kills-trend-container">
    <div v-if="loading" class="loading">Loading kills trend…</div>
    <div v-else-if="error" class="error">{{ error }}</div>
    <div v-else-if="!hasData" class="empty">No kills trend data available.</div>

    <ChartCard v-else title="📈 Kills Trend">
      <div class="trend-content">
        <div class="summary-row">
          <div class="stat">
            <span class="stat-label">Overall Avg</span>
            <span class="stat-value">{{ data.overallAvgKills }}</span>
          </div>
          <div class="stat">
            <span class="stat-label">Recent Avg</span>
            <span class="stat-value" :class="trendClass">{{ data.recentAvgKills }}</span>
          </div>
...
import getDuoKillsTrend from '@/assets/getDuoKillsTrend.js';
...
const maxKills = computed(() => {
  if (!data.value?.dataPoints) return 40;
  const max = Math.max(...data.value.dataPoints.map(d => d.duoKills));
  return Math.ceil(max / 5) * 5 || 40;
});
...
const dataPoints = computed(() => {
  ...
  return pts.map((d, i) => ({ x: getX(i, pts.length), y: getY(d.duoKills), win: d.win }));
});

const rollingLinePoints = computed(() => {
  if (!data.value?.dataPoints) return '';
  const pts = data.value.dataPoints;
  return pts.map((d, i) => `${getX(i, pts.length)},${getY(d.rollingAvgKills)}`).join(' ');
});
...
async function load() {
  ...
  loading.value = true;
  error.value = null;
  try { data.value = await getDuoKillsTrend(props.userId); }
  catch (e) { error.value = e?.message || 'Failed to load kills trend data.'; }
...
<style scoped>
.kills-trend-container { width: 100%; height: 100%; display: flex; flex-direction: column; }
.kills-trend-container :deep(.chart-card) { flex: 1; display: flex; flex-direction: column; }
...
.rolling-line { fill: none; stroke: var(--color-success); stroke-width: 2; }
...
.line-sample { width: 16px; height: 2px; background: var(--color-success); }
</style>
