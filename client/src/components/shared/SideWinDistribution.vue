<template>
  <div class="side-win-distribution-container">
    ...
    <ChartCard v-else title="🗺️ Win Distribution by Side">
      <div class="chart-content">
        ...
        <p class="summary-text">
          Of your <strong>{{ totalWins }}</strong> wins: <strong>{{ sideData.blueWins }}</strong> from blue
          (<strong>{{ sideData.blueWinRate.toFixed(1) }}%</strong>) and <strong>{{ sideData.redWins }}</strong> from red
          (<strong>{{ sideData.redWinRate.toFixed(1) }}%</strong>)
        </p>
        <p class="summary-text">
          You played <strong>{{ sideData.blueGames }}</strong> games on blue side and <strong>{{ sideData.redGames }}</strong> games on red side
        </p>
      </div>
    </ChartCard>
  </div>
...
import { getSideStats } from '@/api/shared.js';
...
const totalWins = computed(() => {
  if (!sideData.value) return 0;
  return sideData.value.blueWins + sideData.value.redWins;
});
