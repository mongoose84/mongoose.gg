...
### MatchHistorySyncJob (BackgroundService)

**Location:** `server/Infrastructure/External/MatchHistorySyncJob.cs`

**Dependencies:**
- `IServiceProvider` - for creating scopes
- `ILogger<MatchHistorySyncJob>` - for logging
...
```csharp
// Register sync job (after existing MatchHistorySyncJob)
...
|-----------|----------------|
| `MatchHistorySyncJob` | BackgroundService that polls for pending accounts and processes them |
