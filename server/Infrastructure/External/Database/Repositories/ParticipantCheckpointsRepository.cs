...
public class ParticipantCheckpointsRepository : RepositoryBase
{
    ...
    public Task UpsertAsync(ParticipantCheckpoint cp)
    {
    ...
    public async Task UpsertBatchAsync(IEnumerable<ParticipantCheckpoint> checkpoints)
    {
    ...
    public async Task<IList<ParticipantCheckpoint>> GetByParticipantIdsAsync(IEnumerable<long> participantIds)
    {
        var ids = participantIds?.Distinct().ToList() ?? [];
        if (ids.Count == 0) return Array.Empty<ParticipantCheckpoint>();
        ...
        return await ExecuteWithConnectionAsync(async (conn, cmd) =>
        {
            ...
            var results = new List<ParticipantCheckpoint>();
            ...
            return (IList<ParticipantCheckpoint>)results;
        });
    }

    public Task<IList<ParticipantCheckpoint>> GetByParticipantAsync(long participantId)
    {
    ...
    private static ParticipantCheckpoint Map(MySqlDataReader r) => new()
    {
        Id = r.GetInt64(0),
