...
internal sealed class TestWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    internal sealed class FakeOverviewStatsRepository : OverviewStatsRepository
    {
        ...
        /// </summary>
        public void AddMatchResult(string puuid, string matchId, bool win, int? lpAfter, long gameStartTime, string? tierAfter = null, string? rankAfter = null)
        {
            if (!_matchesByPuuid.TryGetValue(puuid, out var matches))
            {
            ...
            matches.Add(new MatchResultData(matchId, win, lpAfter, tierAfter, rankAfter, gameStartTime));
        }
